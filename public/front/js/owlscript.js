(function ($) {
  ...
  // items change number for slider display on desktop
   ...
   if ($('.career-business-carousel').length) {
    ...
    //three-item-carousel
   if ($('.our_facility_carousel').length) {
      $('.our_facility_carousel').owlCarousel({
         loop: true,
         margin:0,
         nav: false,
         smartSpeed: 300,
  ...
  var $carousel = $('.four-item-carousel');

  // Initialize with autoplay disabled
  ...
  // Function to play/stop based on visibility
  function toggleAutoplay(entry) {
    if (entry.isIntersecting) {
      $carousel.trigger('play.owl.autoplay', [3000]); // resume
    } else {
      $carousel.trigger('stop.owl.autoplay');         // pause
    }
  }
  ...
  if ('IntersectionObserver' in window) {
    let observer = new IntersectionObserver(function(entries) {
      entries.forEach(toggleAutoplay);
